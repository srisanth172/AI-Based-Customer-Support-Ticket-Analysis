...
class AIService {
  ...
  async callCategorizationAPI(text) {
    ...
    const prompt = [
      ...
      '  "priorityScore": <float between 0.0 and 1.0 representing precise urgency>,',
  ...
  async analyzeTicketWithImage(text, imagePath) {
    try {
      ...
      6. Notifications & Communication
      7. Subscription & Plans
       (If the description is nonsense, irrelevant to customer support, personal, off-topic, or inappropriate, you MUST use "OutOfScope")

      SPAM DETECTION LOGIC:
      You must check two specific things about the uploaded image:
      1. Does the uploaded image genuinely match the complaint description?
      2. Does the uploaded image look AI-generated, fake, manipulated, edited, stock, or irrelevant?
      ...
      JSON schema:
      {
        ...
        "is_fake": boolean,
        "sentiment": "positive|neutral|negative",
        "sentimentScore": <float between -1.0 and 1.0 representing sentiment intensity>,
        "priority": "low|medium|high",
        "priorityScore": <float between 0.0 and 1.0 representing precise urgency level>,
      ...
      try {
        ...
        console.error('Vision AI primary provider failed:', err.message);
        
        // Fallback to OpenRouter if primary provider (Groq) fails
        const orKey = (process.env.OPENROUTER_API_KEY || '').trim();
        if (orKey && apiKey !== orKey) {
          ...
          throw err; // Re-throw if no OpenRouter key or already using it
        }
      }

      const content = response.data?.choices?.[0]?.message?.content;
      if (!content) return { ...(await this.analyzeTicket(text)), isSpam: false };
      ...
      try {
        ...
        return { ...(await this.analyzeTicket(text)), isSpam: false };
      }
      
      // New Strict Spam Logic - Handle both actual booleans and string "true"/"false"
      const matchesDescription = parsed.matches_description === false || parsed.matches_description === 'false' ? false : true;
      const isFake = parsed.is_fake === true || parsed.is_fake === 'true' ? true : false;
      const isSpam = matchesDescription === false || isFake === true;
      const priority = parsed.priority?.toLowerCase() || 'medium';
      const sentiment = parsed.sentiment?.toLowerCase() || 'neutral';

      return {
        sentiment: sentiment,
        sentimentScore: parsed.sentimentScore || (sentiment === 'negative' ? -0.5 : sentiment === 'positive' ? 0.5 : 0),
        priority: priority,
        priorityScore: parsed.priorityScore || (priority === 'high' ? 0.85 : priority === 'medium' ? 0.5 : 0.15),
        category: parsed.category || 'OutOfScope',
        suggestedReply: parsed.suggestedReply || 'Thanks for your message. We are reviewing it now.',
        suggestedSolutions: Array.isArray(parsed.suggestedSolutions) ? parsed.suggestedSolutions : ['Please provide more details.', 'Restart your device.', 'Clear your cache.', 'Contact our team.'],
        reasoning: parsed.reasoning || 'Classified by model output.',
        keywords: Array.isArray(parsed.keywords) ? parsed.keywords : [],
        isValid: parsed.category !== 'OutOfScope',
        isSpam: isSpam,
        isAI: isFake
      };
