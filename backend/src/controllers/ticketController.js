  ...
  try {
    ...
    // ════════════════════════════════════════════════════════════════════════
      ...
      if (photoFile) {
        ...
        if (!isStillMismatch) {
          if (ticket.ticket_ai_status === 'SPAM') {
            // ── Photo now valid → remove spam flag ──
            ticket.ticket_ai_status = 'VALID';
            ticket.reupload_requested = false;
            ticket.category = reAnalysis.category !== 'OutOfScope' ? reAnalysis.category : 'Product Issues';
            ticket.priority = reAnalysis.priority || ticket.priority;
            ticket.aiAnalysis = { ...ticket.aiAnalysis, ...reAnalysis };
            ticket.activityLog.push({ actionType: 'SPAM_CLEARED', message: 'Ticket validated: resubmitted photo matches the issue description.' });
            ticket.messages.push({
              sender: 'bot',
              text: `✅ **Photo Verified!**\n\nYour new photo has been analyzed and matches your reported issue. Our support team will now assist you shortly.`,
              aiVerification: 'Genuine',
              timestamp: new Date()
            });
            await Notification.create({ recipient: 'admin', title: 'Spam Ticket Validated', description: `Ticket ${ticket.ticketId} was validated after the customer resubmitted a valid photo.`, type: 'info', ticketId: ticket.ticketId });
          } else {
             // Valid photo uploaded on normal ticket
             ticket.messages.push({ sender: 'bot', text: `Thanks for the photo! It has been successfully analyzed and attached to your ticket.`, timestamp: new Date() });
          }
        } else {
          // ── Invalid photo uploaded ──
          ticket.ticket_ai_status = 'SPAM';
          ticket.category = 'Spam';
          ticket.verification_attempts = (ticket.verification_attempts || 0) + 1;
          const mismatchReason = reAnalysis.isAI ? 'appears to be AI-generated' : 'does not match the issue description';
          
          if (ticket.verification_attempts >= 2) {
            // Second invalid resubmission
            ticket.internalNotes.push({ text: `Swift AI: Customer failed proof verification twice. Latest image ${mismatchReason}. Admin action required to close.`, timestamp: new Date() });
            await Notification.create({ recipient: 'admin', title: 'Customer failed proof verification twice', description: `Customer has failed verification twice on ticket ${ticket.ticketId}. Admin action required.`, type: 'warning', ticketId: ticket.ticketId });
          } else {
            // First invalid upload
            ticket.reupload_requested = true;
            ticket.messages.push({
              sender: 'bot',
              text: `For verification, please upload a clearer genuine photo showing the exact issue mentioned in your complaint.`,
              aiVerification: 'Mismatch',
              timestamp: new Date()
            });
            ticket.internalNotes.push({ text: `Swift AI: Uploaded image ${mismatchReason}. Ticket marked as spam. Customer prompted to upload valid photo.`, timestamp: new Date() });
            await Notification.create({ recipient: 'admin', title: 'Invalid Image Uploaded', description: `Customer uploaded an invalid image on ticket ${ticket.ticketId}.`, type: 'warning', ticketId: ticket.ticketId });
          }
        }
...
exports.createTicketWithPhoto = async (req, res) => {
  try {
    ...
    console.log('Is AI:', aiAnalysis.isAI);
    console.log('---------------------------');

    const isImageMismatch = aiAnalysis.isSpam === true;
    console.log('isImageMismatch final value:', isImageMismatch);
    const isOutOfScope = aiAnalysis.category === 'OutOfScope' || !aiAnalysis.isValid;

    // REJECT NONSENSE / OUT OF SCOPE CONTENT
    if (isOutOfScope) {
      console.log('Ticket rejected: Description is OutOfScope or nonsense');
      return res.status(400).json({
        message: 'This issue is not within our support scope. Please provide a valid customer support request.',
        isOutOfScope: true
      });
    }

    const ticketAiStatus = isImageMismatch ? 'SPAM' : 'VALID';
    const verificationAttempts = isImageMismatch ? 1 : 0;
    const reuploadRequested = isImageMismatch;

    const botSpamMessage = isImageMismatch
      ? `For verification, please upload a clearer genuine photo showing the exact issue mentioned in your complaint.`
      : null;
    ...
    const ticketData = {
      ...
      title,
      photoUrl,
      status: 'open', // Always allow submission normally
      ticket_ai_status: ticketAiStatus,
      reupload_requested: reuploadRequested,
      verification_attempts: verificationAttempts,
      category: isImageMismatch ? 'Spam' : aiAnalysis.category,
    ...
    res.status(500).json({ message: 'Failed to create ticket', error: error.message, details: error.errors });
  }
};
