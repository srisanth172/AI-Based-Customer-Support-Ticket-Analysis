  ...
  const filteredTickets = useMemo(() => {
    return tickets.filter(t => {
      ...
      if (sortOrder === 'urgency') {
         const getFallbackScore = (p) => p === 'high' ? 0.85 : p === 'medium' ? 0.5 : 0.15;
         const scoreA = a.aiAnalysis?.priorityScore || getFallbackScore(a.priority);
         const scoreB = b.aiAnalysis?.priorityScore || getFallbackScore(b.priority);
              ...
              >
                ...
                <td className="px-4 py-4">
                  <span className={`inline-flex items-center px-2 py-1 rounded-md text-[10px] font-black uppercase border ${getPriorityProps(ticket.priority).className}`}>
                    ...
                    {ticket.priority} ({Math.round((ticket.aiAnalysis?.priorityScore || (ticket.priority === 'high' ? 0.85 : ticket.priority === 'medium' ? 0.5 : 0.15)) * 100)})
                  </span>
                </td>
                <td className="px-4 py-4">
